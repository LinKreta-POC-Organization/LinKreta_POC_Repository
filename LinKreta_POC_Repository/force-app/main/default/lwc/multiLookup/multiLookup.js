...
export default class MultiLookup extends LightningElement {
      ...
      test({ data, error }) {
            if (data) {
                  ...
                  data.forEach(element => {
                        this.selectedRecords.push({ 'recId': element.recId, 'recName': element.recName });
                        this.userIds.push(element.recId);
                        if (this.userIdToSend === '' || this.userIdToSend === null || this.userIdToSend === undefined) {
                              this.userIdToSend += element.recId;
                        }
                        else {
      ...
      searchField(event) {
            ...
            getResults({ ObjectName: this.objectName, fieldName: this.fieldName, value: currentText, selectedRecId: selectRecId,isEnquiry : this.isEnquiry })
                  .then(result => {
